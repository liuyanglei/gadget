...
async function loadData(){byId('overallHealth').textContent='检测中';byId('overallHealth').className='health-pill';try{const response=await fetch('../data/news.json',{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=await response.text();const payload=JSON.parse(text);state.payload=payload;state.articles=Array.isArray(payload.articles)?payload.articles:[];state.visible=25;renderOverview(new Blob([text]).size);renderCategories();renderSettings();renderQuality();renderTable();showToast('后台数据已刷新')}catch(error){console.error(error);byId('overallHealth').textContent='数据加载失败';byId('overallHealth').className='health-pill bad';showToast('无法读取 news.json')}}
